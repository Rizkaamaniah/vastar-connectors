...
func NewGeminiConnector(baseURL, apiKey, model string) (*GeminiConnector, 
error) { 
...
func (c *GeminiConnector) StreamGenerateContent(req GeminiRequest) (<-chan 
string, <-chan error) { 
    ...
    go func() { 
        ...
        if c.apiKey != "" { 
            ...
            if resp.StatusCode != 200 { 
                errCh <- fmt.Errorf("unexpected status code: %d", 
resp.StatusCode) 
                return 
            } 
